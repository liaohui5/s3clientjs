...
// 必须要使用 Virtual Host-Style 的服务商域名后缀白名单
  ...
  constructor(config: S3Config) {
    ...
    const usePathStyle = (() => {
      ...
      try {
        ...
        logger.warn(
          `[S3Storage] Failed to parse endpoint, fallback to Path-Style: ${endpoint}`,
          e as Error,
        );
        return true;
      }
  ...
  async getFileContents(key: string): Promise<Buffer> {
    try {
      const res = await this.client.send(
        new GetObjectCommand({ Bucket: this.bucket, Key: this.buildKey(key) }),
      );
